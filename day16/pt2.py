...
# Part 2:
best_tiles = set()
for pi in p: best_tiles.add(pi) # pick up best path

# also pick up the other alternatives with same score from the priority heap
next_score = score
while True:
    next_score, startx, starty, last_d, p = heapq.heappop(h) # get next item
    if next_score == score:
        for pi in p: best_tiles.add(pi)
    else: break 
